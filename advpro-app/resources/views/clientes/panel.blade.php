<x-app-layout>
    ...
    <div class="w-full rounded-lg shadow-xs overflow-x-auto lg:overflow-visible">
        <div class="w-full rounded-lg shadow-xs overflow-x-auto xl:overflow-visible">
            <table class="w-full whitespace-nowrap min-w-[800px] xl:min-w-full xl:whitespace-normal">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800" x-data="clientesData()" x-init="fetchClientes()">
                    <template x-for="client in clientes" :key="client.id">
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3">
                                <div class="flex items-center text-sm">
                                    <div>
                                        <p class="font-semibold truncate" x-text="client.nombre"></p>
                            ...
                            <td class="px-4 py-3 text-sm truncate" x-text="client.tipo_documento + '-' + client.documento"></td>
                            <td class="px-4 py-3 text-sm">
                                <span
                                    class="inline-block max-w-full truncate px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100"
                                    x-text="client.email"></span>
                            </td>
                            <td class="px-4 py-3 text-sm truncate" x-text="client.telefono"></td>
                            <td class="px-4 py-3 text-sm break-words" x-text="client.direccion"></td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <button @click="editCliente(client)"
                                    ...
                                    <button @click="deleteCliente(client.id)"
                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Delete">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
                                            viewBox="0 0 20 20">
    ...
    <div x-data="modalData()" x-show="openModal" x-cloak
            ...
            <div class="mt-4 mb-6 ">
                ...
                <form @submit.prevent="submitForm" class="max-w-md mx-auto">
                    <div class="mt-2  text-sm">
                        ...
                        <div>
                            <label class="inline-flex items-center  text-sm">
                                <select name="tipo_documento" id="tipo_documento" x-model="form.tipo_documento"
                            ...
                            <label class="inline-flex items-center text-sm">
                                <input name="documento" x-model="form.documento"
                    ...
                    <label class="block mt-2 text-sm">
                        ...
                        <input name="nombre" x-model="form.nombre"
        ...
        function modalData() {
            return {
                ...
                submitForm() {
                    ...
                    let url = this.form.id ? `/api/clientes/${this.form.id}` : '/api/clientes';
                    ...
                    fetch(url, {
                        ...
                        if (response.ok) {
                            this.closeModal();
                            this.$root.__x.$data.fetchClientes();
                        } else {
